...
class PostService {
    constructor(http) {
        this.http = http;
        this.posts = [];
        this.http.get("http://localhost/laravel/public/api").subscribe((posts) => posts.forEach(e => this.posts.push(new _post__WEBPACK_IMPORTED_MODULE_2__["Post"](e.nome, e.titulo, e.subtitulo, e.email, e.mensagem, e.arquivo, e.id, e.likes))));
    }
    salvar(post, file) {
        ...
        this.http.post("http://localhost/laravel/public/api", uploadData, { observe: 'events' }).subscribe((event) => {
            if (event.type == _angular_common_http__WEBPACK_IMPORTED_MODULE_1__["HttpEventType"].Response) {
                let e = event.body;
                this.posts.push(new _post__WEBPACK_IMPORTED_MODULE_2__["Post"](e.nome, e.titulo, e.subtitulo, e.email, e.mensagem, e.arquivo, e.id, e.likes));
            }
        });
    }
    like(id) {
        this.http.get(`http://localhost/laravel/public/api/like/${id}`).subscribe((event) => {
...
module.exports = __webpack_require__(/*! C:\Users\iaraa\Desktop\walace\programas\htdocs\laravel\resources\meuapp\src\main.ts */"./src/main.ts");
